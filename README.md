...
> 
> 단순한 이미지 크롭 도구를 넘어, Gemini LLM이 사용자의 의도를 분석하고 Grounding DINO와 SAM2가 정밀하게 객체를 수확하는 통합 AI 에이전트 플랫폼입니다.
...
1. **Semantic Parsing (Gemini 2.5 Flash):** 사용자의 모호한 자연어 명령(예: "하얀색 차량만 뽑아줘")을 분석하여 타겟 객체와 공간·색상 필터가 담긴 구조화된 JSON을 생성합니다.
2. **Precision Harvesting (Grounding DINO & SAM2):** 분석된 데이터를 바탕으로 Grounding DINO가 텍스트 프롬프트 기반으로 객체를 탐지하고, SAM2 세그멘테이션을 통해 픽셀 단위로 정밀하게 크롭합니다.
3. **Hybrid Infrastructure:** 언어 분석(Brain)은 **Gemini API**로 처리하고, 무거운 연산이 필요한 비전 모델(Worker)은 **로컬 GPU**에서, 사용자 인터페이스는 **Vercel**에서 동작하는 효율적인 하이브리드 구조를 가집니다.
...
- **Brain (LLM):** Gemini 2.5 Flash (`google-genai` API)
- **Worker (Vision):** Grounding DINO (Zero-shot Detection), SAM2 (Segment Anything Model 2)
...
graph TD
    ...
    subgraph AI_Engine [AI Orchestration Layer]
        BE -->|Semantic Analysis| LLM[Gemini 2.5 Flash API]
        LLM -->|Task Planning| Worker[Vision Engine]
        Worker -->|Object Detection| DINO[Grounding DINO]
...
- **컴퓨팅 자원 최적화:** 개인 단위 개발의 한계를 넘기 위해, 언어 분석은 Gemini API로 오프로딩하고 GPU는 비전 모델에 집중시키는 하이브리드 아키텍처 도입
...
- Python 3.10+
- Node.js 18+
- NVIDIA GPU (CUDA 11.8+ recommended for Grounding DINO/SAM2)
- Gemini API Key (`.env`의 `GEMINI_API_KEY`)
